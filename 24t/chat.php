 ...
 { 
    ...
    try {
     ...
     if ($_SESSION["actype"] ==="tutor"){
      $stmt = $conn->prepare("SELECT chat_id , c.stud_id , c.stud_fname , c.stud_lname , c.stud_profilepic FROM chat d , student c WHERE d.tutor_id = :id AND c.stud_id = d.stud_id Order BY `chat_id` DESC");
      $stmt->bindParam(':id',$_SESSION["identity"], PDO::PARAM_STR);
      $stmt->execute();
      }
     if ($_SESSION["actype"] ==="student"){
        $stmt = $conn->prepare("SELECT chat_id , c.t_job , c.tutor_id , c.t_fname , c.t_lname , c.t_profilepic FROM chat d , tutor c WHERE d.stud_id = :id AND c.tutor_id = d.tutor_id Order BY `chat_id` DESC");
        $stmt->bindParam(':id',$_SESSION["identity"], PDO::PARAM_STR);
        $stmt->execute();
      }
    }
    ...
    <div class="col-sm-9">
        ...
        <div class="card">
            <ul class="nav nav-tabs" role="tablist">
                <li role="presentation" class="active"><a href="#home" aria-controls="home" role="tab" data-toggle="tab">Chat Messages</a></li>
            </ul>
            ...
            <div class="tab-content">
                <div role="tabpanel" class="tab-pane active" id="home">
                    ...
                    foreach ($stmt as $row) {
                      ...
                      echo "<div class='box box1'><a href='chatting.php?chat=".$row['chat_id']."&img=".$row['t_profilepic']."&name=".$name."'><img src='uploads/".$row['t_profilepic']."'  height='80' width='80'></a></div>";
                      echo "<div class='box box2'><a href='chatting.php?chat=".$row['chat_id']."&img=".$row['t_profilepic']."&name=".$name."'><b>".$row['t_fname']." ".$row['t_lname']."</b></a><br>".$row['t_job']."</div>";
                      }   
                    }
                    if ($_SESSION["actype"] ==="tutor"){
                      foreach ($stmt as $row) {
                        $name = $row['stud_fname']." ".$row['stud_lname'];
                        echo "<div class='box box1'><a href='chatting.php?chat=".$row['chat_id']."&img=".$row['stud_profilepic']."&name=".$name."'><img src='uploads/".$row['stud_profilepic']."'  height='80' width='80'></a></div>";
                        echo "<div class='box box2'><a href='chatting.php?chat=".$row['chat_id']."&img=".$row['stud_profilepic']."&name=".$name."'><b>".$row['stud_fname']." ".$row['stud_lname']."</b></a><br></div>";
                      }   
                    }
                    ?>
